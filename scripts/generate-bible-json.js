#!/usr/bin/env node
// Converts the downloaded KJV JSON (thiagobodruk format) into per-book files
// matching our required structure: { name, chapters: { "1": [{verse, text}] } }
...
const sourceFile = path.join(rootDir, "kjv_full.json");
const outputDir = path.join(rootDir, "public", "bible");
...
let rawText = fs.readFileSync(sourceFile, "utf8");
...
if (rawData.length !== 66) {
  console.error(`Expected 66 books, got ${rawData.length}`);
  process.exit(1);
}
...
for (let i = 0; i < 66; i++) {
  ...
  const rawBook = rawData[i];

  // rawBook.chapters is an array of arrays of verse strings
  const chapters = {};
  rawBook.chapters.forEach((chapterVerses, chapterIdx) => {
    const chapterNum = String(chapterIdx + 1);
    chapters[chapterNum] = chapterVerses.map((text, verseIdx) => ({
      verse: verseIdx + 1,
      text: text,
    }));
  });
  ...
  const outPath = path.join(outputDir, `${bookMeta.slug}.json`);
  fs.writeFileSync(outPath, JSON.stringify(output));
  written++;
  console.log(`Written: ${bookMeta.slug}.json (${rawBook.chapters.length} chapters)`);
}

console.log(`\nDone! Wrote ${written} book files to public/bible/`);
