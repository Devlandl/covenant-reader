...
export async function loadChapter(bookSlug: string, chapter: number) {
  const res = await fetch(`/bible/${bookSlug}.json`);
  const data = await res.json();
  return data.chapters[String(chapter)] as { verse: number; text: string }[];
}
